...
## templates/lenses/comment-cleanup/SKILL.md
- Upstream project: `motlin/claude-code-plugins` (public, discovered via mcpmarket.com's listing for "clean-code-comments," but fetched directly from this authoritative upstream source rather than from mcpmarket's own rendered/cached copy, which was found to be stale relative to it -- different `description` text, and missing the "Choose cleanup scope" section entirely)
- Source: `plugins/code/skills/comment-cleanup/SKILL.md`
- Source revision: commit `e82e75e1d3097d04727f7b907b4ef467f4f0a989` (2026-07-18)
- License: Apache-2.0 (confirmed via the repository's root `LICENSE` file)
- Relationship: Vendored near-verbatim. Content is pure comment-hygiene guidance (diff-only vs. repository-wide cleanup scope, what clutter to remove, what to preserve, end-of-line comment placement) with no runner-specific tool coupling, no CLI invocations, and no ce-harness-specific paths to adapt. `SKILL.md`'s frontmatter and body are otherwise unchanged; a trailing `_See THIRD_PARTY_NOTICES.md..._` line was appended to match ce-harness's existing skill-provenance convention. Consumed as an external Agent Skill lens (a subdirectory's own `SKILL.md`, discovered and selected by `/verify`/`/adversarial-review` exactly like any native ce-harness lens) rather than rewritten as a native ce-harness lens -- its guidance is applied for finding identification only, never as license to edit or fix, since it is authored in an editing voice for an agent that makes changes directly.

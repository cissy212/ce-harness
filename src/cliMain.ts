  ...
  program
    ...
    .description(
      [
        ...
        "branch tip (see `detectBaseBranch`; never assumes \"main\"), fetched",
        "fresh from the remote first so a new workspace is always established",
        "from the current remote base, never a possibly-stale local branch of",
        "the same name. Pass --from to instead start this same kind of",
        "Implementation workspace from any other resolvable ref (a local",
        "branch, an origin/<branch> remote-tracking ref, a tag, or a raw",
        "commit) -- e.g. a completed dependency branch that hasn't merged to",
        "the default branch yet; --from is never fetched, and must already",
        "exist locally. Pass --base and --head together to instead review an",
        "exact commit range (e.g. an existing pull request), which creates an",
        "Existing PR review workspace, not an Implementation one -- --from and",
        "--base/--head are mutually exclusive, and --base/--head must also",
        "already exist locally. Multiple",
